...
2. `build` signs, notarizes, staples, verifies, and uploads `MacVM-<version>.dmg` and `MacVM-<version>.pkg` after the test gate passes.
...
That branch runs ordinary tests, then builds signed and notarized disk-image and package artifacts. It does not create a GitHub Release.
