#!/usr/bin/env python3
"""Tests for the MacVM release helper."""
...
def write_project(root: pathlib.Path, version: str = "1.0.0") -> None:
...
class ReleaseTestCase(unittest.TestCase):
    ...
    def test_patch_from_no_tags_resolves_initial_project_version(self) -> None:
        self.assertEqual(release_tool.resolve_version(self.root, "patch"), "1.0.0")

    def test_resolves_patch_from_plain_tag(self) -> None:
        git(self.root, "tag", "-a", "v1.0.0", "-m", "MacVM v1.0.0")

        self.assertEqual(release_tool.resolve_version(self.root, "patch"), "1.0.1")

    def test_update_marketing_version_updates_all_three_part_versions(self) -> None:
        count = release_tool.update_marketing_version(self.root, "1.0.1")

        contents = (self.root / release_tool.PROJECT_FILE).read_text()
        self.assertEqual(count, 4)
        self.assertEqual(contents.count("MARKETING_VERSION = 1.0.1;"), 4)
        self.assertNotIn("MARKETING_VERSION = 1.0.0;", contents)

    def test_release_commits_project_update_and_creates_tag(self) -> None:
        git(self.root, "tag", "-a", "v1.0.0", "-m", "MacVM v1.0.0")
        ...
        self.assertEqual(git_output(self.root, "tag", "-l", "v1.0.1"), "v1.0.1")
        self.assertIn("MARKETING_VERSION = 1.0.1;", (self.root / release_tool.PROJECT_FILE).read_text())
        self.assertEqual(git_output(self.root, "log", "-1", "--pretty=%s"), "chore(release): bump version to v1.0.1")
    ...
    def test_initial_release_does_not_commit_when_project_already_matches(self) -> None:
        ...
        self.assertEqual(git_output(self.root, "tag", "-l", "v1.0.0"), "v1.0.0")
    ...
    def test_dry_run_does_not_require_clean_worktree(self) -> None:
        ...
        self.assertEqual(git_output(self.root, "tag", "-l", "v1.0.1"), "")
        self.assertIn("MARKETING_VERSION = 1.0.0;", (self.root / release_tool.PROJECT_FILE).read_text())
    ...
    def test_rejects_versions_with_v_prefix(self) -> None:
        with self.assertRaisesRegex(release_tool.ReleaseError, "must not start with 'v'"):
            release_tool.resolve_version(self.root, "v1.0.1")
    ...
    def test_pushed_release_requires_head_on_origin_main(self) -> None:
        ...
        self.assertEqual(git_output(self.root, "tag", "-l", "v1.0.0"), "")
