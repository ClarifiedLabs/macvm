#!/usr/bin/env python3
"""Release helper for MacVM GitHub release tags."""
...
PROJECT_FILE = pathlib.Path("macvm.xcodeproj/project.pbxproj")
...
def project_version(root: pathlib.Path) -> str:
    ...
    match = MARKETING_VERSION_VALUE_RE.search(project_file.read_text())
    if not match:
...
def print_plan(plan: ReleasePlan) -> None:
    print(f"Bumping MacVM: {plan.current_version} -> v{plan.new_version}")
...
def create_release(root: pathlib.Path, version_arg: str, dry_run: bool, push: bool) -> None:
    ...
    git(root, "tag", "-a", plan.tag, "-m", f"MacVM v{plan.new_version}")
    ...
    print()
    print("To trigger the GitHub release workflow:")
    if committed:
        print("  git push")
...
def parser() -> argparse.ArgumentParser:
    root_parser = argparse.ArgumentParser(description="Release MacVM to GitHub Releases.")
