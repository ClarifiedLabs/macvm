...
# same virtualization entitlement as the CLI. CONFIG=Debug produces a signed
# debug app for iteration. Public Developer ID packaging is handled by
...
case "$CONFIG" in
  ...
  release|Release)
    ...
    ;;
  *)
    echo "Unsupported CONFIG '$CONFIG' (expected Debug or Release)" >&2
    exit 2
    ;;
esac
